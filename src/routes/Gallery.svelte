...
<div id="gallerySection" class="flex flex-col justify-start w-full mt-12">
  ...
  <div
    class="w-full h-[450px] mt-6 rounded-md bg-radial from-background-secondary to-background overflow-hidden relative border border-background-tertiary"
  >
      ...
      {#key currentIndex}
        ...
        >
          <div class="relative m-14 mt-8">
            ...
            <div
              class="absolute -bottom-8 left-1/2 -translate-x-1/2 text-faint text-xs text-center font-medium min-w-[300px]"
            >
    ...
    <div class="flex space-x-2 absolute bottom-2 right-2">
      <button
        onclick={goPrev}
        aria-label="goLeftGallery"
        class="w-8 h-8 cursor-pointer bg-background hover:bg-background-secondary/50 text-[#EEEEE4] rounded-full flex items-center justify-center border border-background-tertiary"
      >
